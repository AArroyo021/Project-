Feature: A Manager should be able to log in to view the reimbursements of all Employees
  ...
  Scenario: As a Manager I want to Approve a reimbursement request because it seems valid
    ...
    When The Manager inputs Approve RadioButton for ClaimStatus and Manager Validation, and inputs their Manager Reasoning
    When The Manager clicks the Update Button
    Then The Manager gets a Approve Updated Successfully Alert and the page reloads

    Scenario: As a Manager I want to Deny a reimbursement request because it seems sketchy
    #Given The Manager is on the reimbursement page
    When The Manager clicks on the Pending Button again
    When The Manager clicks on the Claim Number Button again
    When The Manager inputs Deny RadioButton for ClaimStatus and Manager Validation, and inputs their Manager Reasoning
    When The Manager clicks the Update Button again
    Then The Manager gets a Deny Updated Successfully Alert and the page reloads
